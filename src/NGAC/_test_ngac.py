...
if os.getcwd().endswith("NGAC"):
    from ngac import *
else:
    from NGAC.ngac import *


def test_ngac():
    """
    Starts and stops the NGAC server

    This is a basic test to ensure that the NGAC server can be started and stopped
    """
    ngac = NGAC()
    ngac.start()
    ngac.stop()
...
def test_with_ngac():
    """
    Test the NGAC class in a with statement

    This should ensure that the NGAC server is stopped
    and never left running
    """
    # This creates a new NGAC instance, starts it, and then stops it
    # This should wrap the entire function
    with NGAC() as ngac:
        assert ngac is not None
...
def test_combine_policies():
    ...
    with NGAC() as ngac:
        ...
        # Load the policies
        policyA = Policy(
            "Signals Access Policy", path="EXAMPLES/policy_signals_access.pl"
        )
        policyB = Policy(
            "Vehicle Ownership Policy", path="EXAMPLES/policy_vehicle_ownership.pl"
        )
...
def test_load_policy():
    ...
    with NGAC() as ngac:
        ...
        policy = Policy("CondPolicy1", path="./EXAMPLES/condpolicy1.pl")
        time.sleep(2)
...
def test_set_get_policy():
    ...
    with NGAC() as ngac:
        ...
        print(ngac.switch_to(Policy("Policy (b)"), token="admin_token").text)
        assert ngac.get(Policy, token="admin_token").text.split("\n")[0] == "Policy (b)"

        print(ngac.switch_to(Policy("Policy (a)"), token="admin_token").text)
        assert ngac.get(Policy, token="admin_token").text.split("\n")[0] == "Policy (a)"
